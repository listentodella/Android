...
#if WORKAROUND_BUG_15000108
        // FIXME: Some drivers are manually setting the buffer count
        // (which they shouldn't), so let's *IGNORE* it
        const int maxBufferCount = mCore->getMaxBufferCountLocked(false);
        if (bufferCount <= maxBufferCount) {
            BQ_LOGV("setBufferCount: request buffer count %d ignored", bufferCount);
            return NO_ERROR;
        }
#endif
