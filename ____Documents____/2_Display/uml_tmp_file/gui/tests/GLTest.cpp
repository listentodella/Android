...
void GLTest::loadShader(GLenum shaderType, const char * pSource, GLuint * outShader)
{
...
void GLTest::createProgram(const char * pVertexSource, const char * pFragmentSource, GLuint * outPgm)
{
...
const EGLint * GLTest::getConfigAttribs() {
...
const EGLint * GLTest::getContextAttribs() {
...
EGLSurface GLTest::createWindowSurface(EGLDisplay display, EGLConfig config, sp<ANativeWindow> & window) const {
    return eglCreateWindowSurface(display, config, window.get(), NULL);
}

::testing::AssertionResult GLTest::checkPixel(int x, int y, int r, int g, int b, int a, int tolerance) {
    GLubyte pixel[4];
    String8 msg;
...
::testing::AssertionResult GLTest::assertRectEq(const Rect & r1, const Rect & r2, int tolerance) {
    String8 msg;
