#ifndef ANDROID_SRGBTEST_H
#define ANDROID_SRGBTEST_H
...
class SRGBTest : public typedef3 {
  public:
    enum  {
      DISPLAY_WIDTH =  512,
      DISPLAY_HEIGHT =  512,
  ...
  protected:
    ...
    static inline uint8_t srgbToLinear(uint8_t u) {
            float f = u / 255.0f;
            return static_cast<uint8_t>(srgbToLinear(f) * 255.0f + 0.5f);
        };

    inline void fillTexture(bool writeAsSRGB) {
    ...
    inline void drawTexture(bool asSRGB, GLint x, GLint y, GLsizei width, GLsizei height) {
    ...
    inline void checkLockedBuffer(PixelFormat format) {
    ...
    static inline bool withinTolerance(int a, int b) {
            int diff = a - b;
  ...
  private:
    inline void createEGLSurface(Surface * inputSurface) {
    ...
    inline void createDebugSurface() {
    ...
    inline void copyToDebugSurface() {
