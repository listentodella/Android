...
/*r-led1  g-led2 b-led3 */
/*这些灯的值都由一个config.xml来设置区间  
...
static int set_light_backlight (struct light_device_t *dev, struct light_state_t const *state) {
	...
	ALOGV("hml %s brightness=%d color=0x%08x", __func__,brightness,state->color);
	pthread_mutex_lock(&g_lock);
	/*brightness 0~255
     * but LCD_BACKLIGHT_FILE can bear 0~255  
     */
